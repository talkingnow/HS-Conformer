...
def run(process_id, args, experiment_args):
    ...
    test_set_LA = data_processing.TestSet(asvspoof.test_set_LA, args['test_crop_size'])
    test_sampler = DistributedSampler(test_set_LA, shuffle=False)
    test_loader_LA = DataLoader(
        test_set_LA,
        num_workers=args['num_workers'],
        batch_size=args['batch_size'],
        pin_memory=True,
    ...
    # frontend
    frontend = egg_exp.framework.model.ConformerMP_NonLPE_HieraCLS2(bin_size=args['bin_size'], output_size=args['output_size'], input_layer=args['input_layer'],
    ...
    best_state_LA = framework.copy_state_dict()
    cnt_early_stop = 0
    ...
    for epoch in range(1, args['epoch'] + 1):
            ...
            if eer < best_eer_DF:
                ...
                if logger is not None:
                    ...
                    for key, v in best_state_ft.items():
                        logger.save_model(f'check_point_DF_{key}_{epoch}', v)
            if logger is not None:
        ...
        # test_LA
        if epoch % 5 == 0:
            eer = train.test(framework, test_loader_LA)
            ...
            if eer < best_eer_LA:
                ...
                best_state_ft = framework.copy_state_dict()
                if logger is not None:
                    logger.log_metric('BestEER_LA', eer, epoch)
                    for key, v in best_state_ft.items():
                        logger.save_model(f'check_point_LA_{key}_{epoch}', v)
            if logger is not None:
                logger.log_metric('EER_LA', eer, epoch)
