...
class ConformerEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        x: torch.Tensor,
        pos_emb: torch.Tensor,
        mask_pad: Optional[torch.Tensor] = None,    # remove
    ):
        ...
        # Temporal multi-headed self-attention module 
        residual = x
        if self.normalize_before:
            x = self.norm_mha(x)
        
        x_att_T = self.self_attn(x, x, x, None, pos_emb) 
        # x = residual + self.dropout(x_att)
        
        if not self.normalize_before:
            x_att_T = self.norm_mha(x_att_T)
        # --> x.size = (B, F, T, H)

        # Spartial multi-headed self-attention module 
        x_S = x_att_T.permute(0, 2, 1, 3)   # (B, T, F, H)

        if self.normalize_before:
            x_S = self.norm_mha(x_S)
        
        F_pos_emb = pos_emb.reshape(1, x.size(1), x.size(2), x.size(3)).permute(0, 2, 1, 3).reshape(1, -1, x.size(3))
        x_att_F = self.self_attn(x_S, x_S, x_S, None, F_pos_emb)    
        x_att_F = x_att_F.permute(0, 2, 1, 3)
        # --> (B, F, T, H)

        x = residual + self.dropout(x_att_F)
        if not self.normalize_before:
            x = self.norm_mha(x)
