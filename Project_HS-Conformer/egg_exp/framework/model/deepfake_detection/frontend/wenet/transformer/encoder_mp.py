...
from wenet.transformer.encoder_layer_mp import ConformerEncoderLayerMP
...
class BaseEncoder(torch.nn.Module):
    ...
    def forward(
        ...
        # print('base_encoder', xs.size(), pos_emb.size())
        i = 0 
        for layer in self.encoders:
            # print(i, 'layer')
            xs, pos_emb = layer(xs, pos_emb)
            # print(i, xs.size(), pos_emb.size())
            i += 1
    ...
    def __init__(
        ...
        for i in range(num_blocks):
            if i in downsample_layer: 
                ...
                downsample = None
                seq_len = seq_len
            encoders.append(ConformerEncoderLayerMP(
                    output_size,
                    seq_len,
                    downsample,
