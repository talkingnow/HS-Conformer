...
class ConformerEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        if self.feed_forward_macaron is not None:
            ...
            x = residual + self.ff_scale * self.dropout30(
        ...
        x = residual + self.dropout35(x_att)
        if not self.normalize_before:
            x = self.norm_mha(x)
        ...
        x = residual + self.ff_scale * self.dropout30(self.feed_forward(x))
        if not self.normalize_before:
            x = self.norm_ff(x)
