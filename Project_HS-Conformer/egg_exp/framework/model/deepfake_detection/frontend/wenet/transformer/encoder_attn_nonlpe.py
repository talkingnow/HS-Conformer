...
from wenet.transformer.encoder_layer_attn_nonlpe import ConformerEncoderLayerAttn_NonLPE
    ...
    def __init__(
        ...
        for i in range(num_blocks):
            if i in downsample_layer: 
                downsample = AttnPooling(output_size, attn_size=attn_size)
                seq_len = math.ceil(seq_len / attn_size)
            else:
                downsample = None
                seq_len = seq_len
            encoders.append(ConformerEncoderLayerAttn_NonLPE(
                    output_size,
                    seq_len,
                    downsample,
...
class AttnPooling(torch.nn.Module):
    ...
    def forward(self, x):
        # size setting: (B,T,H) -> (B,t,attn_size,H)
        B, T, H = x.size()
        t = T // self.attn_size
        x_1 = x[:, : t * self.attn_size, :].reshape(B, t, self.attn_size, H)
        x_2 = x[:, -self.attn_size:, :].unsqueeze(1)
        x = torch.cat((x_1,x_2), dim=1)

        # attention module
        attn_w = self.fc(x)
        attn_w = self.softmax(attn_w.squeeze(-1)).unsqueeze(-1)   # B,t,a,1
