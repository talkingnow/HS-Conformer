...
from wenet.transformer.encoder_layer_conv_nonlpe import ConformerEncoderLayerConv_NonLPE
    ...
    def __init__(
        ...
        for i in range(num_blocks):
            if i in downsample_layer: 
                downsample = torch.nn.Conv1d(output_size, output_size, kernel_size=kernel_size, stride=stride)
                seq_len = math.floor( (seq_len - kernel_size -2) / stride + 1)
            else:
                downsample = None
                seq_len = seq_len
            encoders.append(ConformerEncoderLayerConv_NonLPE(
                    output_size,
                    seq_len,
                    downsample,
