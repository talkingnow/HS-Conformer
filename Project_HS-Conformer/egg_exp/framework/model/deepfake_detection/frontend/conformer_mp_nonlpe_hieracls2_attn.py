...
class ConformerMP_NonLPE_HieraCLS2_Attn(nn.Module):
    def __init__(self, bin_size=120, num_blocks=6, output_size=128, input_layer="conv2d2", 
            ...
            downsample_layer=[1,3], pooling_size=2, input_seq_len=200, layer_cls=True, dropout=0, emb_dropout=0, multiloss=False, tanh=True):

        super(ConformerMP_NonLPE_HieraCLS2_Attn, self).__init__()
        ...
        self.emb_attn = SelfWeightedPooling(output_size, num_head=1, mean_only=True)
    ...
    def forward(self, x):
        ...
        cls0 = self.emb_dropout(self.bn0(self.silu(self.fc0(cls[:, 0, :])))).unsqueeze(1)
        cls1 = self.emb_dropout(self.bn1(self.silu(self.fc1(cls[:, 1, :])))).unsqueeze(1)
        cls2 = self.emb_dropout(self.bn2(self.silu(self.fc2(cls[:, 2, :])))).unsqueeze(1)
        
        seq_emb = self.emb_dropout(self.attn(x)).unsqueeze(1)
        
        embedding = torch.cat((cls0, cls1, cls2, seq_emb), dim=1)
        output = self.dropout(embedding)
        output, attn_w = self.emb_attn(output, get_w=True, tanh=self.tanh)
        
        if self.multiloss:
            return output.unsqueeze(1), embedding, attn_w
        return output
...
#     model = ConformerMP_NonLPE_HieraCLS2_Attn().cuda()
